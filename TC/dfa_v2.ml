...
let next simb maquina memo =
  ...
  let () = Printf.printf "Estou no next %d\n" (List.length res) in 
...
let is_accepted memo maq =
  ...
  (List.exists (fun x -> mem x accept) laqui) && restante=[]
...
let output maquina dados=  
  ...
  try
    ...
    while true do
      let () = Printf.printf "Estou aqui\n" in 
      memor := (step !memor maquina);
      let (est,fi) = !memor in
        let () = Printf.printf "estado -> ";
        List.iter (fun e -> Printf.printf "%d " e) est; Printf.printf "\n";
        Printf.printf "fita -> ";
        List.iter (fun e -> Printf.printf "%c " (match e with | None -> ' ' | Some c -> c)) fi;
        Printf.printf "\n" in ()
    done
  with
...
1 -> em cada estado que estou verificar:
  - o buffer acabou?
    - estou num estado final?  Sim ou Nao
    - existem transições epsilon? percorrer essas caso existam
  - Se buffer nao acabou, ir à lista de transições e filtrar pelas transições que partem do estado em que estou pelo primeiro caracter do buffer ou por epsilon
    - em seguida percorrer essas transicoes


List.iter (fun x -> Printf.printf "%d " x) caminho; Printf.printf "\n"
