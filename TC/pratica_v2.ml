...
(*let rec principal_NDFA biglist cont_biglist estados_fin ltrans palavra cont caminho aqui=
  ...
  *)
  ...
  (*let rec executa palavra cont caminho aqui possiveis =
    ...
    [] -> is_accepted palavra acabou e estamos no estado final Printf.printf "\n"
    | h::t -> Printf.printf "%d " h; executa palavra cont caminho aqui t
  *)
  ...
  let rec executa palavra cont caminho aqui possiveis =
    ...
    | h::t -> let aux_step = step trans aqui h palavra cont caminho  in 
              let (a,b,c) = aux_step in
              let aux_next = next trans a (palavra.[b]) in
              let h = executa palavra b c a aux_next in
              let (d,e)=h in
